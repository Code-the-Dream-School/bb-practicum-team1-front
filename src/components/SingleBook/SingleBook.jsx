...
const SingleBook = () => {
	...
	const fetchSingleBookById = async (id) => {
		if (!id) {
			setIsError(true);
			setErrorMessage('There is no book ID input');
		}

		if (id) {
			setLoading(true);
			const newBook = await getSingleBookAdapter(id);
		...
		<div className='container-single'>
			<div className='book-item-single'>
				...
				<p className='kidsAdults-single'>
					{adult ? (
						<img
							src={Adults}
							alt='Adults'
							title='Not appropriate for under 13 years old'
						/>
					) : (
						<img
							src={ZeroPlus}
				...
				{noImg ? (
					<img
						src={NoPic}
						alt='No_Picture_available'
						className='coverImage-single'
					/>
				) : (
					<img
						src={image}
				...
				<div className='book-item-body-single'>
					<div>
						...
						<Link
							to={`/chat/${bookInformation?.owner?._id}`}
							className='link-to-owner'
							data-id='Contact the owner'>
							<img
								src={Message}
								alt='message_me'
							/>
